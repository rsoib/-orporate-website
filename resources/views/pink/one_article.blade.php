<div id="content-single" class="content group">
	<div class="hentry hentry-post blog-big group ">
		@if($article)
				                ...
				                <div class="thumbnail">
				                    <!-- post title -->
				                    <h1 class="post-title">{{ $article->art_title }}</h1>
				                    <!-- post featured -->
				                    <div class="image-wrap">
				                        <img src="{{ asset('assets') }}/images/articles/{{ $article->art_img->max }}" alt="00212" title="00212" />        
				                    </div>
				                    <p class="date">
				                ...
				                <div class="meta group">
				                    <p class="author"><span>by <a href="" title="{{ $article->art_title }}" rel="author">{{ $article->user->name }}</a></span></p>
				                    <p class="categories"><span>In: <a href="{{ route('articlesCat',['cat_alias'=>$article->category->cat_alias]) }}" title="View all posts in {{ $article->category->cat_title }}" rel="category tag">{{ $article->category->cat_title }}</a></span></p>
                            ...
                            @endif    
				                ...
				                <div id="respond">
				                    ...
				                    <form action="{{ route('comment.store') }}" method="post" id="commentform">
				                        @if(!Auth::check())
											<p class="comment-form-author"><label for="name">Name</label> <input id="name" name="com_name" type="text" value="" size="30" aria-required="true" /></p>
				                        <p class="comment-form-email"><label for="email">Email</label> <input id="email" name="com_email" type="text" value="" size="30" aria-required="true" /></p>
				                        <p class="comment-form-url"><label for="url">Website</label><input id="url" name="com_site" type="text" value="" size="30" /></p>
				                        
				                        @endif

				                        <p class="comment-form-comment"><label for="comment">Your comment</label><textarea id="comment" name="com_text" cols="45" rows="8"></textarea></p>
				                        <div class="clear"></div>
				                        <p class="form-submit">
											{{ csrf_field() }}
